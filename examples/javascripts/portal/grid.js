Ext.ux.InteractiveGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	initComponent: function() {
		...
		Ext.ux.InteractiveGridPanel.superclass.initComponent.apply(this, arguments);
	},
	
	updatePrice: function(g,r) {

		var row = this.store.getAt(r);
		var obj = row.data;
	...
	onDataUpdate: function(m) {
		...
		m.data.lastChange = new Date();

		var matches = this.store.query("company", m.data.company);
		if(matches.length <= 0) {
			this.addRecord(obj);
		} else {
			var record = matches.items[0];
			this.store.recordType.prototype.fields.each(function(f) {
